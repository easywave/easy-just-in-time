...
char easy::IndirectcallConstant::ID = 0;

llvm::Pass* easy::createIndirectcallConstantPass(llvm::StringRef Name) {
  return new IndirectcallConstant(Name);
}
...
bool DeindirectCall(IIter it, IIter end) {
  bool Changed = false;
  for(; it != end; ++it) {
    Instruction &I = *it;
...
static bool CastCallWithPointerCasts(FunctionType* CalledTy, FunctionType* UncastedTy) {
...
template<class IIter>
void RecastIndirectCalls(IIter it, IIter end) {
  for(;it != end;) {
    CallSite CS{&*it++};
    if(!CS)
      continue;
...
bool easy::IndirectcallConstant::runOnFunction(llvm::Function &F) {
  ...
  for(auto it = inst_begin(F); it != inst_end(F); ++it) {
    ...
    Value* Called = CS.getCalledValue();
    auto* Location = dyn_cast<Constant>(Called->stripPointerCasts());
    if(!Location)
      continue;
    ...
    if(auto* CE = dyn_cast<ConstantExpr>(Location)) {
      if(CE->getOpcode() == Instruction::IntToPtr) {
        ...
        if(!CLocation)
          continue;
        auto Addr = CLocation->getSExtValue();
        auto NewF = findFunctionAndLinkModules(*F.getParent(), (void*)Addr);
...
static RegisterPass<easy::IndirectcallConstant> X("","",false, false);
